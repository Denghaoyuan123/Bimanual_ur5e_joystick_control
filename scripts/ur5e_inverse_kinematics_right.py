...
class UR5eInverseKinematics:
    def __init__(self):
        rospy.init_node('ur5e_inverse_kinematics1', anonymous=True)
        ...
        self.gripper_publisher = rospy.Publisher('/gripper_state1', Float64MultiArray, queue_size=1)

        self.joint_angles_received = False  # Flag to check if joint angles are received
        self.subscription_joy = rospy.Subscriber('/joy1', Joy, self.joystick_callback)
        self.subscription_joint_states = rospy.Subscriber('/robot1/joint_states', JointState, self.joint_states_callback)
        self.publisher = rospy.Publisher('/robot1/joint_group_vel_controller/command', Float64MultiArray, queue_size=1)
    ...
    def joystick_callback(self, msg):
        if not self.joint_angles_received:
            rospy.loginfo('Robot 1 Joint angles not received yet, skipping inverse kinematics calculation.')
            return
